...
| Каталог | Вміст | Бекап |
|---|---|---|
| `data/duty/` | Графіки по місяцях + внутрішній git-журнал усіх змін | **Так** — це і є дані |
...
| `data/caldav-state/` | Кеш стану синку | Ні, відновлюється сам |

Оскільки `data/duty/` — це повноцінний git-репозиторій, найпростіший
бекап графіка це `git clone` чи `git bundle` з нього: разом із даними
поїде вся історія правок.
...
локально — в `./data/duty`, у production — у змонтованому томі.

Внутрішній git-журнал графіка ніяк не пов'язаний з репозиторієм цього
коду: він живе всередині `data-dir`, разом із самими даними.
