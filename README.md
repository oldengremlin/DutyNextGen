...
## Обмін чергуваннями (у розробці)

Черговий інженер зможе запропонувати іншому черговому обмін датами —
чергування на чергування (Ч↔Ч) чи звичайний робочий день на робочий день
(Р↔Р), ніколи мішані — типовий сценарій: кілька вихідних поспіль без
втрати робочих годин. Прийнята обома сторонами пропозиція йде на
затвердження адміністратору, і лише після цього застосовується до
графіка — одним git-комітом від імені саме ініціатора пропозиції (не
того, хто погодився, і не адміністратора).

Поки що готовий лише внутрішній шар зберігання: пропозиції — текстові
файли з git-історією в `data/exchanges/` (`DUTY_EXCHANGES_DIR`), той
самий підхід, що й у `data/templates/`. Веб-інтерфейс і бізнес-логіка
(валідація обміну, застосування до графіка) — окремими наступними
версіями.
