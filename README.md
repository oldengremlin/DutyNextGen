...
браузерне «Друкувати» / Ctrl+P ховає навігацію і друкує в альбомній
орієнтації.
...
- **Що публікується.** Для поточного й наступного місяця — по одній
  події на кожну активну позначку (D/W/O/I/S) кожного адміністратора,
  від сьогодні й далі (минуле не публікується навмисно, як і в старому
  скрипті): чергування (D) — 8:00–20:00, робочий день (W) — 9:00–17:30
  ...
  продовжує (не задвоює) уже опубліковані в Baikal події.
- **Коли.** `CalDavSyncScheduler` — фоновий аналог старого
  ...
  до 5 хвилин.
- **Ідемпотентність і видалення.** PUT лише якщо вміст події змінився
  (за хешем — `net.ukrhub.duty.caldav.CalDavSyncState`, окремий файл
  стану на місяць у `DUTY_CALDAV_STATE_DIR`); DELETE подій, чиї
  позначки зникли з графіка — **крім уже минулих днів, їх ніколи не
  видаляємо** (той самий захист, що й у старому скрипті: інакше подія,
  яка щойно випала з "сьогодні й далі", виглядала б як "зникла" й
  стиралась би).
