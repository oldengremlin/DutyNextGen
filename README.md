...
вихідні й сьогоднішній день підсвічені. Друк — тим самим шаблоном, через
`@media print` (окремої сторінки/кнопки навмисно нема — простіше в
підтримці, і завжди синхронно з екранною версією): браузерне
«Друкувати» / Ctrl+P ховає навігацію і друкує в альбомній орієнтації.
...
(`DutyScheduleGenerator`) — підтримує рівно двох "чергових" адміністраторів
(без ознаки «лише робочі дні»); решта просто працює в будні (Р) і має
вихідний у суботу/неділю. Продовжує ротацію двох чергових з того місця,
де вона зупинилась минулого місяця: наприкінці кожного місяця в файлі
зберігаються позначки чергових за два останні дні
(`[ LastDay0 ]`/`[ LastDay1 ]`), і генератор шукає в жорстко зашитому
44-денному ротаційному шаблоні (той самий, що й у `tds.pl`, подвоєний до
88 записів), де ці дві позначки йдуть поспіль — знайдена фаза й визначає
ротацію на наступний місяць, день за днем. Якщо вхідні дані не
відповідають передумовам (не рівно два чергових інженери, чи фазу не
вдалось визначити за зіпсованими `[ LastDay0 ]`/`[ LastDay1 ]`) —
...
- **Кнопка «Згенерувати {наступний місяць}»** на сторінці перегляду —
  ...
  існує (`POST /schedule/{ym}/generate-next`). Прив'язана до конкретного
  переглянутого місяця `ym`, а не до "сьогодні": можна перейти на вже
  згенерований жовтень і згенерувати з нього листопад, і так по
  ланцюжку на скільки завгодно місяців уперед.
- **Фоновий job** (`ScheduleGenerationScheduler`, `@Scheduled`, раз на
  добу о 03:00) — аналог старого cron + `monthly-duty`-демона: якщо
  графіка на місяць, наступний за поточним реальним, ще нема — генерує
  його на основі поточного місяця. Кнопка вище — резерв на випадок, якщо
  ця перевірка колись не спрацює (наприклад, застосунок був вимкнений),
  і спосіб згенерувати наперед більше ніж на один місяць одразу.
...
**У системі завжди має лишатися хоча б один адміністратор** — `/admin/users`
не дає видалити самого себе (`UserAdminController.delete`) і не дає
понизити роль єдиного адміністратора (`UserAdminController.changeRole`) —
обидві перевірки на рівні контролера, а не лише відсутність відповідної
дії в UI. Це навмисно захищає саме від випадкового самозамикання (єдиний
адмін помилково прибирає собі роль чи видаляє себе) — від навмисного
одночасного видалення/пониження ролі кількома різними адміністраторами
одразу воно не рятує (і не повинно: якщо адмінів двоє й більше — рішення
позбутися одного з них цілком свідоме).
