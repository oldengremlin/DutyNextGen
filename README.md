...
команди. Подвійний захист: JVM запускається з
`-Djdk.lang.Process.launchMechanism=FORK` (Dockerfile) замість типового
`posix_spawn`, а `GitCommitService` додатково повторює `pb.start()` до
трьох разів перед тим, як здатись.
