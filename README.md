...
падає транзиентно (`IOException: Exec failed`) ще до запуску самої
команди — і саме на командах з додатковими env-змінними (автор/дата
коміту), що вказує на пошук `git` через `PATH` у зібраному для процесу
оточенні. Кілька незалежних заходів одразу: `GitCommitService`
викликає `git` за абсолютним шляхом (без пошуку через `PATH`) і
повторює `pb.start()` до п'яти разів із наростаючою паузою; JVM
додатково запускається з `-Djdk.lang.Process.launchMechanism=FORK`
(Dockerfile) замість типового `posix_spawn`. Якщо і це не допомогло —
`DutyExchangeController` перехоплює збій git-шару на діях над обміном
чергуваннями й показує дружнє повідомлення замість Whitelabel Error
Page.
