...
позначок. Створення графіка на новий місяць — окрема функція (генерація
наступного місяця, ще не реалізована).
...
`ім'я:bcrypt-хеш:роль`, по одному користувачу на рядок (рядки без ролі,
записані до появи цього поля, трактуються як Адміністратор — попередня
поведінка єдиного користувача, без пониження прав при оновленні). Цей
