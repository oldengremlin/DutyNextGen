...
(`tools/import-cvs-history.pl`, виконано одноразово в монорепо `Duty` до
виділення цього репозиторію, дивись `CHANGELOG.md` щодо деталей — зокрема,
чому 166 найстаріших місяців мають лише один коміт кожен). Оригінальний
CVS-репозиторій (`safe/duty/cvs/cvsroot/` у репозиторії `Duty`) лишається
недоторканим як довідкове джерело.

Подивитись історію конкретного місяця:

```sh
git log --follow -- data/duty/202409
```
