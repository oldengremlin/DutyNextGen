...
форма редагування наявного місяця: позначки по днях (Ч/Р/В/Л/С/вихідний),
ім'я й ознака «лише робочі дні» для кожного адміністратора. Зберігає
`POST` на той самий шлях — один git-коміт на збереження, з іменем
користувача, який зайшов через Basic Auth. Створення графіка на новий
місяць — окрема функція (генерація наступного місяця, ще не реалізована).
Додавання чи видалення адміністраторів з ростеру через веб поки не
підтримується.

Свято (успадкований з застарілого проєкту маркер `*` після дня тижня,
напр. 1 травня) підсвічується в таблиці окремим кольором — новий/змінений
графік цю позначку зберігає, як і раніше.
